-- ST6IX V1.3 HDR: photographic post-processing for Assetto Corsa
-- Pure Gamma 3.50 / CSP HDR output (DXGI flip model + HDR support)
...
function init_pure_script()
    ...
    header('🏁 ST6IX V1.3 HDR · PHOTOGRAPHIC PP FILTER', 'Every slider is live in every mode. Pick a look, then fine-tune.')
    pure.script.ui.addText('📺 HDR: Windows HDR on | CSP: DXGI flip model + HDR support | AC windowed/borderless')
    header('🎯 SIGNATURE LOOKS', 'Set each tab as shown, or mix and match.')
    pure.script.ui.addText('🌅 CLEAR MORNING - Daytime→ Bright Morning | Sky→ Natural')
    pure.script.ui.addText('   Glare→ Gentle')
    pure.script.ui.addText('🏁 RACE BROADCAST - Daytime→ Photorealistic | Glare→ GT Broadcast')
    pure.script.ui.addText('   HDR&Clarity→ Full HDR')
    pure.script.ui.addText('🌇 GOLDEN HOUR - Daytime→ Cinematic Morning | Sky→ Golden Hour')
    pure.script.ui.addText('   Glare→ Golden Hour | Reflections→ Cinematic')
    pure.script.ui.addText('🌧️ WET NIGHT - Night→ Cinematic Night | Glare→ Wet Night')
    pure.script.ui.addText('   Reflections→ Cinematic')
    pure.script.ui.addText('🌌 COSMIC NIGHT - Night→ Dark Night | Skydomes→ ST6IX Nebula')
    pure.script.ui.addText('   Glare→ Cinematic | HDR&Clarity→ Subtle HDR')
    pure.script.ui.addText('📸 PHOTO MODE - LensFX→ Cinematic + Depth of Field | Film Grain on')
    pure.script.ui.addText('   Exposure→ Photography | Lock Exposure for the shot')
    header('💡 TIPS')
    pure.script.ui.addText('Full HDR☀️ suits most daytime, Subtle HDR🌤️ keeps nights clean.')
    pure.script.ui.addText('Cinematic Sharp🎥 is made for replays and screenshots.')
    pure.script.ui.addText('Diagnostics tab: live readings when something looks off.')

    pure.script.ui.addPage('☀️Daytime')
    header('🎬 OVERALL LOOK', 'Modes add a subtle finish; every slider stays live.')
    pure.script.ui.addRadioButtons('Overall Mode', 2, 'Natural🌿,Photorealistic📸,Manual⚙️')
    header('🌅 MORNING LOOK', 'Starting look for the daylight hours')
    pure.script.ui.addRadioButtons('Morning Preset', 1, 'Natural Morning🌿,Bright Morning☀️,Dark Morning🌑,Cinematic Morning🎬,Custom⚙️')
    ...
    header('🌡️ WHITE BALANCE', 'Adapts to time, clouds, fog and rain within safe limits')
    ...
    header('🌙 NIGHT LOOK')
    pure.script.ui.addRadioButtons('Night Preset', 1, 'Natural Night🌙,Bright Night🌕,Dark Night🌑,Cinematic Night🎬,Custom⚙️')
    header('🏙️ LIGHT POLLUTION & AMBIENT')
    ...
    header('🔭 CELESTIAL REALISM', 'Moon and stars react to elevation, clouds, fog and city light')
    ...
    header('🌤️ SKY LOOK', 'Weather refines the look with smooth transitions')
    pure.script.ui.addRadioButtons('Sky Preset', 1, 'Natural🌿,Golden Hour🌇,Overcast Soft☁️,Pastel Dawn🌅,City Haze🏙️,Storm⛈️,Custom⚙️')
    ...
    header('🌌 CUSTOM SKYDOMES', 'Replace the sky with ST6IX cosmic backgrounds')
    pure.script.ui.addRadioButtons('Skydome Preset', 1, 'Off🌍,ST6IX Nebula🌌,ST6IX Madara🔴,ST6IX Black Hole🕳️,ST6IX Black Matter⚫,ST6IX Black Matter Colour🟣')
    pure.script.ui.addText('↳ Off: normal sky | Others: ST6IX skydome textures')
    header('🎨 SKYDOME APPEARANCE')
    slider('Skydome Brightness', 1.00, 0.10, 5.00, 'Skydome luminosity')
    slider('Skydome Contrast', 1.00, 0.10, 3.00, 'Skydome colour contrast')
    header('📍 SKYDOME POSITION')
    slider('Skydome Rotation', 0.50, 0.00, 3.00, 'Horizontal rotation of the skydome')
    slider('Skydome Height', 1.00, 0.50, 2.00, 'Vertical position of the skydome')

    pure.script.ui.addPage('🌫️Fog')
    header('🌫️ FOG FINE TUNING', 'Scales Pure\'s live weather fog; transitions are preserved')
    ...
    pure.script.ui.addRadioButtons('Fog Color Preset', 1, 'Weather Adaptive🌦️,Neutral⚪,Cold Morning❄️,Warm Sunset🌇,Storm⛈️,Night🌙')
    ...
    header('✴️ BLOOM', 'Restrained, source-aware bloom')
    ...
    pure.script.ui.addRadioButtons('Glare Style', 1, 'Gentle🌤️,GT Broadcast📺,Cinematic🎬,Le Mans🏁,Lens Flare💫,Wet Night🌧️,Golden Hour🌇,Custom⚙️')
    slider('Glare Style Strength', 0.85, 0.00, 1.00, 'Blend the selected style with the manual glare controls')
    header('⚡ REACTIVE GLARE', 'Adapts to exposure, highlights and time of day')
    ...
    header('🚇 TUNNELS', 'Normal is neutral; the others adapt inside and recover at the exit')
    pure.script.ui.addRadioButtons('Tunnel Preset', 1, 'Normal🚗,Flash⚡,Blinding☀️,Cinematic🎬')
    ...
    header('🧠 SMART EXPOSURE')
    pure.script.ui.addCheckbox('Adaptive Scene Exposure', true, 'Use scene brightness, occlusion and weather to refine exposure')
    pure.script.ui.addRadioButtons('Exposure Metering', 1, 'Balanced⚖️,Center Weighted🎯,Cockpit Weighted🏎️,Photography📸')
    ...
    pure.script.ui.addPage('🎨HDR Tonemap')
    header('🎨 HDR OUTPUT', 'CSP performs the final HDR tone mapping for your display.')
    pure.script.ui.addText('Enable DXGI flip model + HDR support in CSP; run AC windowed or borderless.')
    header('🧠 SCENE AWARE HDR', 'Shapes the scene before CSP maps it to your display')
    ...
    header('🔍 HDR & CLARITY', 'HDR-grade sharpness, clarity and colour depth')
    pure.script.ui.addRadioButtons('Clarity Preset', 1, 'Off⭕,Subtle HDR🌤️,Full HDR☀️,Ultra HDR🔥,Cinematic Sharp🎥,Manual⚙️')
    pure.script.ui.addText('↳ Presets set the sliders below; choose Manual⚙️ to set them yourself.')
    header('🔪 SHARPNESS')
    slider('Sharpness', 0.35, 0.00, 1.00, 'Edge definition and fine detail; eased at night and in the cockpit')
    header('💎 CLARITY & LOCAL CONTRAST')
    slider('Clarity', 0.45, 0.00, 1.00, 'Midtone contrast for a vivid, HDR-like look')
    slider('Micro Contrast', 0.22, 0.00, 0.50, 'Fine texture detail')
    header('✨ HDR ENHANCEMENT')
    slider('Brightness Boost', 1.14, 0.80, 1.50, 'Widens the highlight range')
    slider('Color Vibrance', 0.18, 0.00, 0.50, 'Lifts muted colours more than saturated ones')
    slider('Highlight Recovery', 0.28, 0.00, 0.60, 'Lowers exposure slightly when bright highlights dominate')

    pure.script.ui.addPage('📹LensFX')
    header('📹 LENS PROFILE', 'Physical lens character')
    pure.script.ui.addRadioButtons('Lens Profile', 1, 'Clean✨,Cinematic🎬,Vintage📼,Telephoto🔭,Custom⚙️')
    slider('Lens Profile Strength', 0.70, 0.00, 1.00, 'Blend the selected physical lens response')
    header('☀️ GOD RAYS', 'Sun, camera and field-of-view aware')
    ...
    header('🎬 FILM GRAIN')
    pure.script.ui.addCheckbox('Film Grain', false, 'Analog film grain texture for a cinematic look')
    slider('Film Grain Strength', 1.00, 0.00, 10.00, 'Film grain intensity; slightly finer in daylight')
    header('📸 DEPTH OF FIELD', 'For replays and screenshots')
    ...
    pure.script.ui.addRadioButtons('Reflection Preset', 1, 'Normal🚗,Cinematic🎬,Photography📸,Custom⚙️')
    ...
    header('🌧️ WET WEATHER', 'Wet roads, standing water and night lights')
    ...
    header('🔧 DIAGNOSTICS', 'Live scene readings and warnings')
    pure.script.ui.addCheckbox('Diagnostics Enabled', false, 'Update live scene readings and warnings')
    pure.script.ui.addRadioButtons('Effect Preview', 1, 'Normal👁️,Exposure📷,Glare💫,Reflections✨,Fog🌫️')
...
function update_pure_script(dt)
    ...
    -- CSP applies the final HDR tone mapping, so the scene-aware HDR response
    -- steers highlights, shadows and fog through exposure and colour instead.
    local hdrStrength = check('Scene Aware HDR', true)
        and number('HDR Adaptation Strength',0.45,0,1) or 0
    local hdrEV = number('HDR Brightness',0,-1.5,1.5)
        - highlightSignal * number('HDR Highlight Protection',0.55,0,1) * hdrStrength * 0.50
        + darknessSignal * number('HDR Shadow Lift',0.35,0,1) * hdrStrength * 0.35
    local hdrContrast = number('HDR Contrast',1,0.85,1.2)
        * (1 + math.max(worldFog, mist) * number('HDR Fog Contrast',0.40,0,1) * hdrStrength * 0.08)
    pure.config.set('pp.saturation', colorSat * finish.saturation * number('HDR Saturation',1,0.8,1.2), true)
    pure.config.set('pp.contrast', contrast * finish.contrast * hdrContrast, true)
    ...
    -- CSP's HDR output expects a linear YEBIS tone function with neutral gamma
    -- and performs the final display mapping itself.
