-- ST6IX V1.3: photographic post-processing for Assetto Corsa
...
function init_pure_script()
    ...
    header('🏁 ST6IX V1.3 · PHOTOGRAPHIC PP FILTER', 'Every slider is live in every mode. Pick a look, then fine-tune.')
    ...
    pure.script.ui.addText('   Glare→ Gentle | Tonemap→ Silver Screen')
    pure.script.ui.addText('🏁 RACE BROADCAST - Daytime→ Photorealistic | Glare→ GT Broadcast')
    pure.script.ui.addText('   HDR&Clarity→ Full HDR | Tonemap→ Grand Tour')
    ...
    pure.script.ui.addText('   Reflections→ Cinematic | Tonemap→ Neon Punch')
    ...
    header('🎨 TONE CURVE', 'Silver Screen: filmic | Grand Tour: broadcast | Neon Punch: punchy')
    pure.script.ui.addRadioButtons('Tone Curve', 1, 'Silver Screen🎞️,Grand Tour🏁,Neon Punch⚡')
    slider('Tonemap Gamma', 1.10, 0.80, 1.40, 'Final display gamma')
    header('🧠 SCENE AWARE TONE', 'Refines the curve for highlights, darkness and fog')
    ...
    header('🎞️ SILVER SCREEN', 'AgX film curve: soft highlights, natural colour')
    ...
    header('🏁 GRAND TOUR', 'Uchimura curve, the Gran Turismo tonemapper')
    ...
    header('⚡ NEON PUNCH', 'Lottes curve: bold contrast and colour')
...
function update_pure_script(dt)
    ...
    pure.config.set('pp.saturation', colorSat * finish.saturation, true)
    pure.config.set('pp.contrast', contrast * finish.contrast, true)
